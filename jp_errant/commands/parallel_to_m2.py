...
def main():
    ...
    annotator = jp_errant.load(args.lang)
    ...
    with ExitStack() as stack, open(args.out, "w", encoding="utf-8") as out_m2:
        ...
        for line in zip(*in_files):
            ...
            # If corrected texts in TSV format (separated by tab character) -> split the references
            cors_combined = []
            for cor_item in cors:
                cors_combined.extend(cor_item.split("\t"))
            cors = cors_combined    
...
def parse_args():
    parser=argparse.ArgumentParser(
        ...
        usage="%(prog)s [-h] [options] -orig ORIG -cor COR [COR ...] [-tsv yes] -out OUT")
    parser.add_argument(
        "-orig",
    ...
    parser.add_argument(
        ...
        # default="eng")
        help="The 2-letter language code (default: en).",
        default="en")
    parser.add_argument(
        "-lev",
