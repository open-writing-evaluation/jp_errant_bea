...
def main():
    ...
    hyp_m2, ref_m2 = errant.align_m2(hyp_m2, ref_m2) # DEBUG, disable this line if there's no jpalign package
...
def parse_args():
    ...
    parser.add_argument(
        "-st",
        "--strategy",
        help="Choose comparing strategy in multi references situation.\n"
            "top (default): Choose the best (by F-score, TP, FP, etc) reference among multiple ones. \n"
            "avg: Calculate average of local TP, FP, FN counts among references for global counts.\n"
            "wavg: Calculate weighted average of local TP, FP, FN counts among references for global counts.\n"
            "merged: Merge all references then calculate TP, FP, FN counts towards global counts. ",
        choices=["top", "avg", "wavg", "merged"],
        default="top"),
...
def evaluate_edits(hyp_dict, ref_dict, best, sent_id, original_sentence, args):
    ...
    # For "top" couting strategy: best_f is initialised as -1 cause 0 is a valid result. 
    best_tp, best_fp, best_fn, best_f, best_hyp, best_ref = 0, 0, 0, -1, 0, 0
    best_cat = {}
    # For "avg" / "wavg" counting strategy: store FP, FP, FN counts of *all* references 
    # then calculate average or weighted average counts
    all_tp, all_fp, all_fn, all_f_score = [], [], [], []
    ...
    for hyp_id in hyp_dict.keys():
        for ref_id in ref_dict.keys():
            ...
            # For "avg" / "wavg" counting strategy: store TP, FP, FN counts
            all_tp.append(tp)
            all_fp.append(fp)
            all_fn.append(fn)
            all_f_score.append(f)
    ...
    if args.strategy == "top":
        if args.verbose:
            ...
            # print(f"Debug: best_cat =", best_cat)
            print("Local results:")
    ...
    elif args.strategy == "avg" or args.strategy == "wavg":
        # "avg" / "wavg" stragegy: calculate average / weighted average of TP/FP/FN counts across all references
        
        # Average function
        average_fn = lambda numbers: sum(numbers) / len(numbers) if len(numbers)>0 else 0
        assert average_fn([]) == 0
        assert average_fn([1, 2, 3]) == 2
        assert average_fn([1, 2, 3, 4]) == 2.5

        # Softmax function, for weighted average
        softmax_fn = lambda numbers: (np.exp(numbers) / sum(np.exp(numbers))) if sum(np.exp(numbers) > 0) else 0 
        assert np.allclose(softmax_fn([0, 0, 0, 0]), [0.25, 0.25, 0.25, 0.25])
        assert np.allclose(softmax_fn([0, 1, 2, 3]), [0.03, 0.09, 0.24, 0.64], atol=0.01)
        assert sum(softmax_fn([1, 2, 3, 4, 5, 6, 7])) == 1
    
        # Weighted average function
        weighted_average_fn = lambda numbers, weights: np.dot(numbers, weights)
        # assert np.isclose(softmax_weighted_average_fn([1, 1, 1]), 1, atol=0.01)
        # assert np.isclose(softmax_weighted_average_fn([4, 2, 3]), 3.575, atol=0.01)        
        
        weights = []
        if args.strategy == "wavg": 
            weights = softmax_fn(np.multiply(all_f_score,10))
            average_function_apply = lambda numbers: weighted_average_fn(numbers, weights)
        else:
            average_function_apply = average_fn
            
        if best_f < 1:
            avg_tp = average_function_apply(all_tp)
            avg_fp = average_function_apply(all_fp)
            avg_fn = average_function_apply(all_fn)
            avg_dict = {"tp":avg_tp, "fp":avg_fp, "fn":avg_fn}
        else:
            # If there is a perfect match (fscore=1) with *any* reference, 
            # ignore the average and award full score for the sentence
            avg_tp = best_tp
            avg_fp = best_fp
            avg_fn = best_fn
            avg_dict = {"tp":best_tp, "fp":best_fp, "fn":best_fn}
            
        
        if args.verbose:
            if best_f >= 1:
                print(f"A perfect match found with hypothesis = {best_hyp} and reference = {best_ref}. Awarded full score for the sentence.")
            message = "Weighted average" if args.strategy == "wavg" else "Average"
            if len(weights) > 0:
                weights_rounded = [round(value, 2) for value in weights]
                print(f"Weights for each reference is {weights_rounded}")
            print(f"TP counts = {all_tp}. {message} TP count = {avg_tp:.3f}")
            print(f"FP counts = {all_fp}. {message} FP count = {avg_fp:.3f}")
            print(f"FN counts = {all_fn}. {message} FN count = {avg_fn:.3f}")
        return avg_dict, best_cat
    elif args.strategy == "wavg":
        pass
    elif args.strategy == "merged":
        pass
...
def print_results(best, best_cats, args):
    ...
    print("\t".join(map(lambda number: f"{number:.3f}", [best["tp"], best["fp"],
